...
switch lower(bending)
    case 'single'
        ...
        displGB = (((fyM/(L/1000)-Vc1)/kscrb)/1000+(Vc1/kseff)/1000)+fycurv * (((L+Lsp(i))/1000)^2)/3 + (2*min(fy/Es,0.002)*Lpw*L*(40*Bcore^2*Es*betaw*etac/(7*L0^2*fy)-1)/H)/1000*((((fyM/(L/1000)-Vc1)/kscrb)/1000+(Vc1/kseff)/1000)/(fycurv * (((L+Lsp(i))/1000)^2)/3)+1);      %displacement capacity at the onset of wall lateral instability
    case 'double'
        ksg   = (G*As/(L/2))/1000;
        kscrgb  = (kscr1/(L/2));
        Lpw = min(0.15*L/2,0.5*H);                              %wall effective plastic hinge length
        L0 = min((0.2*H+ 0.044*L/2),0.8*hn);                    %critical buckling length of wall
        displGB = (((2*fyM/(L/1000)-Vc1)/kscrb)/1000+(Vc1/kseff)/1000)+fycurv * (((L+2*Lsp(i))/1000)^2)/3 + (2*min(fy/Es,0.002)*Lpw*L*(40*Bcore^2*Es*betaw*etac/(7*L0^2*fy)-1)/H)/1000*((((2*fyM/(L/1000)-Vc1)/kscrb)/1000+(Vc1/kseff)/1000)/(fycurv * (((L+2*Lsp(i))/1000)^2)/3)+1);   %displacement capacity at the onset of wall lateral instability
end
